...
Toggle `House` to show a dog house on the far right. When the dog reaches it, there is a 33% chance it will sleep there for 10-30 seconds. While sleeping, the dog disappears and the house shows animated cartoon `Z` letters. If a `pause` signal arrives while it is asleep, the dog becomes visible again so the red blinking pause state is still obvious. Clicking the house while the dog is asleep wakes it and triggers the spin animation before it moves again.
